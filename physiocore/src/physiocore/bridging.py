...
class BridgingTracker:
    def __init__(self, config_path=None, debug=False, render_all=False, save_video=False, test_mode=False):
        self.debug, self.render_all, self.save_video, self.test_mode = debug, render_all, save_video, test_mode
        self.video = None
        self.config = self._load_config(config_path or self._default_config_path())
        self.hold_secs = self.config.get("HOLD_SECS", 0.1)
    ...
    def _load_config(self, path):
        try:
            with open(path) as conf:
                ...
                if self.test_mode and "HOLD_SECS" in config:
                    del config["HOLD_SECS"]
                return config
    ...
    def start(self):
        ...
        self.process_video(self.video if self.video else 0)
