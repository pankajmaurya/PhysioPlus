...
def detect_feet_orientation(landmarks):
    lankle = landmarks.landmark[mp_pose.PoseLandmark.LEFT_ANKLE.value]
    lfoot_ind = landmarks.landmark[mp_pose.PoseLandmark.LEFT_FOOT_INDEX.value]
    rankle = landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ANKLE.value]
    rfoot_ind = landmarks.landmark[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX.value]
    lheel = landmarks.landmark[mp_pose.PoseLandmark.LEFT_HEEL.value]
    rheel = landmarks.landmark[mp_pose.PoseLandmark.RIGHT_HEEL.value]
...
def upper_body_is_lying_down(landmarks):
    ...
    ground_level = max([landmarks.landmark[lm].y for lm in body_landmarks])

    tolerance = 0.9 * torso_length - 0.015
    # Check if the person is lying down
    #max_y_distance = max(abs(landmarks.landmark[lm].y - ground_level) for lm in body_landmarks)
    lying_down = all(abs(landmarks.landmark[lm].y - ground_level) <= tolerance for lm in body_landmarks)
...
def lower_body_on_ground(landmarks, check_knee_angles=False):
    ...
    on_ground = all(abs(landmarks.landmark[lm].y - ground_level) <= tolerance for lm in body_landmarks) and knee_check_passed
