...
class TestGraphicsUtils(unittest.TestCase):
    ...
    def test_pause_loop_resume(self):
        mock_wait_key = MagicMock(return_value=ord('r'))
        result = pause_loop(wait_key_func=mock_wait_key)
        self.assertFalse(result)
        mock_wait_key.assert_called_once_with(0)

    def test_pause_loop_quit(self):
        mock_wait_key = MagicMock(return_value=ord('q'))
        cleanup_callback = MagicMock()
        result = pause_loop(cleanup_callback=cleanup_callback, wait_key_func=mock_wait_key)
        self.assertTrue(result)
        mock_wait_key.assert_called_once_with(0)
        cleanup_callback.assert_called_once()

    def test_pause_loop_quit_no_callback(self):
        mock_wait_key = MagicMock(return_value=ord('q'))
        result = pause_loop(wait_key_func=mock_wait_key)
        self.assertTrue(result)
        mock_wait_key.assert_called_once_with(0)
